...
def solution(k, dungeons):
    route = [i for i in range(len(dungeons))]
    dungeonRoutes = permutations(route)

    maxDungeon = 0
    maxHp = k

    for dungeonRoute in dungeonRoutes:
        tmpDungeon = 0
        k = maxHp
        for stage in dungeonRoute:
            minNP = dungeons[stage][0]
            np = dungeons[stage][1]
            if k >= minNP:
                tmpDungeon += 1
                k -= np
        if maxDungeon < tmpDungeon:
            maxDungeon = tmpDungeon
