...
def rotate(gears, commandList):
    for gearNum, dir in commandList:
        if dir == 1:
            x = gears[gearNum].pop()
            gears[gearNum].appendleft(x)
        elif dir == -1:
            x = gears[gearNum].popleft()
            gears[gearNum].append(x)
...
for gearNum, dir in commandList:
    ...
    for i in range(gearNum, 0, -1):
        j = i - 1
        if gears[i][6] == gears[j][2]:
            break
        dir = -dir
        cmdList.append([j, dir])
    for i in range(gearNum, T - 1):
        j = i + 1
        if gears[i][2] == gears[j][6]:
            break
        dir2 = -dir2
        cmdList.append([j, dir2])
    rotate(gears, cmdList)
...
문제
총 8개의 톱니를 가지고 있는 톱니바퀴 T개가 아래 그림과 같이 일렬로 놓여져 있다. 또, 톱니는 N극 또는 S극 중 하나를 나타내고 있다.
톱니바퀴에는 번호가 매겨져 있는데, 가장 왼쪽 톱니바퀴가 1번, 그 오른쪽은 2번, ..., 가장 오른쪽 톱니바퀴는 T번이다. 아래 그림은 T가 4인 경우이다.
