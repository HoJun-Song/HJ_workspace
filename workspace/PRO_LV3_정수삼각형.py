def solution(triangle):
    triLength = len(triangle)
    if triLength == 1:
        return triangle[0][0]

    maxTri = [[0] * (i + 1) for i in range(triLength)]
    maxTri[0][0] = triangle[0][0]
    maxTri[1][0] = maxTri[0][0] + triangle[1][0]
    maxTri[1][1] = maxTri[0][0] + triangle[1][1]

    for i in range(2, triLength):
        for j in range(i + 1):
            if j == 0:
